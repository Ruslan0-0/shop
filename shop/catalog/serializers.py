...
from catalog.models import Category, Product, ProductImage, Discount, Seller, Promocode, Cashback
from datetime import date
from catalog.models import OrderProducts, Order
...
class DiscountSerializer(serializers.ModelSerializer):
    class Meta:
        model = Discount
        fields = ('id', 'percent', 'name', 'date_start', 'date_end')
...
class CartSerializer(serializers.Serializer):
    products = ProductInSerializer(many=True)
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if promocode:
            ...
            if use_cashback:
                allow_to_pay_by_points = total_sum * (100 - cashback.percent) / 100

                if user.cashback_points >= allow_to_pay_by_points:
